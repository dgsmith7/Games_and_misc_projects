...
void setup () {
  ...
  best3 = new digit();
  best4 = new digit();
  copyrightInfo = new billBoard ("Copyright (c) 2014 by David Gail Smith", messageColor, 110, 670);
...
void mouseClicked () {
  // if its a control click figure out which one
  if ((mouseY >= 460) && (mouseY <= 475)) {  // Clicked on a button
  ...
  } else if ((solutionFound == false) && (mouseY<460)) { // If solution found, no more move available
    // if its a tile click, get the array position  
    int rowHolder = 4;
    int colHolder = 4;
    for (int j=0; j<4; j++) {
      for (int k=0; k<4; k++) {
    ...
    rearrangeBoard (colHolder, rowHolder);
    solutionFound = checkSolved ();
    if (solutionFound == true) {
      solutionFound = true;
      puzzleSolvedInfo.display ();
...
void rearrangeBoard(int col, int row) {
...
void drawMoveCounter (int moves) {
...
void drawNumber(int which, float x, float y) {
...
// returns first digit for numbers 0-99
{
  if (number <10) {
    return 0;
  } else {
...
// returns second digit for numbers 0-99
{
  if (number <10) {
    return number;
  } else {
